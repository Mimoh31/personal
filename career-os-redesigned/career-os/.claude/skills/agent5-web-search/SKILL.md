...
## Being triggered from the dashboard's "Run" button

The Job feed screen has a "Run web search agent" button. It can't
call Claude directly — clicking it appends a request to
`data/agent-requests.json` with `{"agent": "agent5-web-search",
"username": ..., "requestedAt": ...}`. When starting a Claude Code
session, check this file for unhandled requests targeting this agent
and process them the same as if the user had typed "search for jobs"
directly. There's no need to write anything back to that file — it's
just a request log, not a status channel.

## Inputs (read-only)

1. `data/guidance/<username>.json` — written by Agent 1. **Check this
   first.** If it exists, use it and don't second-guess it.
2. If guidance does not exist yet, **fall back to reading the resume
   directly**: read every file in `cv-library/<username>/` and derive
   a lightweight, ad-hoc version of guidance yourself — target roles,
   must-haves, obvious deal-breakers — just for this search run. This
   is intentionally lighter-weight than what Agent 1 would produce
   (no need for the full strengths/gaps analysis), and it doesn't get
   written to `data/guidance/<username>.json` — that file is Agent
   1's alone to create. Tell the user in your summary that this run
   used an ad-hoc read of the resume, not full guidance, and that
   running Agent 1 first would give better-targeted results.
3. `data/jobs.json` — the existing job store, used only for dedup
