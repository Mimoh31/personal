# Career OS — Phase 0 through 6, redesigned
...
**First time only:** double-click `start.bat`. This installs Flask
and confirms everything works — you'll briefly see a console window
for this one-time setup.

**Every time after:** double-click `Start Career OS.vbs`. No console
window at all — it starts the server completely hidden and opens
your browser straight to the login screen. To close it, double-click
`Stop Career OS.vbs`. This is a real local app: your data stays on
your machine, nothing is hosted anywhere, and it costs nothing to
run. I wrote and reasoned through both VBS scripts carefully, but I
don't have a Windows machine in the environment I built this in — you
are effectively my test environment for this part. If either doesn't
behave as expected the first time, tell me exactly what happened and
I'll debug it with you.

## What changed in this redesign pass

- **Login and registration are now separate pages**, not a toggled
  card. Fixed a real bug along the way: the two screens were
  overlapping in the page (register was pushed below the visible
  viewport, not properly hidden) due to a CSS specificity conflict —
  caught by actually rendering the page and inspecting it at a taller
  viewport, not by reading the CSS.
- **A landing dashboard** now appears immediately after login, before
  Profile — quick stats (jobs found, shortlisted, CV ready, applied,
  guidance status) plus shortcut tiles to every section.
- **Visual redesign**: ink-charcoal sidebar, warm paper content area,
  a single deep-teal accent, Fraunces for headings, Inter for body
  text, and IBM Plex Mono for every number and status readout —
  treating the job search like an instrument panel reading out its
  own state.
- **Job feed has a "Run web search agent" button.** It can't call
  Claude directly — no button in a web page can, without a paid API
  key, which was explicitly off the table — so it queues a request in
  `data/agent-requests.json` and tells you to say "search for jobs"
  in Claude Code. Agent 5's skill now checks that queue on start.
- **Agent 5 checks guidance first, always** — per your decision, it
  does not fall back to reading the resume directly if guidance is
  missing.
- **The Flask server no longer needs a visible terminal.** Debug mode
  is off (this also fixes the double-process "restarting with
  watchdog" noise from before), and the VBS launcher runs it via
  `pythonw.exe`, which has no console window at all.
